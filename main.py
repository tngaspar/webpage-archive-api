...
app=FastAPI(title="SaveMyPage API", description="An API so save and archive pages for later access")
...
def _archive(url):
    try:
        ...
        print("Unable to generate archive URL for " + url)
...
async def add_url(title:str, url:str):
    ...
    db.insert({"title":title, "url":url, "timestamp":datetime.now().timestamp()})
...
async def get_all_url2(request: Request):
    # fetches all records from db
    all_urls=db.fetch()
    return templates.TemplateResponse("index.html", {"request": request, "items": all_urls.items})
